...
onMounted(() => {
    newBlogStore.getBlogPosts(searchValue.value,
        searchValue.value,
        currentPage.value,
        rowsPerPage.value)
});
...
<template>
    <div class="'mainDiv'">
        <div>
            ...
            <p class="'subHeader'">A blog about food, experiences, and recipes</p>
            <div
        ...
        <div :style="{ minWidth: 'auto', maxWidth: '98%' }">
            <div class="mb-10 grid x-sm:grid-col-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4 3xl:grid-cols-5 gap-10 mt-10"
                ...
                <div v-for="(blog) in posts" :key="blog.id" :class="'w-full'">
                    <BlogPost :posted-by="blog?.user?.username" :id="blog.id" :title="blog.title" :imgUrl="blog.image"
                        :subtitle="blog.subtitle" :comments="blog.comments" />
                </div>
            </div>
        ...
        }" :rows="rowsPerPage" :totalRecords="blogStore.totalRecords" @page="onPageChange">
        </Paginator>
    </div>
</template>
<style>
